...
(defun conflito-linear (estado)
  (let ((valor 0)
        (estado-atual (cdr estado))
        (posicao-suposta nil)
        (outro-valor nil)
        (posicao-outro-valor nil)
        (valor-atual nil))
    (dotimes (i (array-dimension estado-atual 0) valor)
      (dotimes (j (array-dimension estado-atual 1))
        (setf valor-atual (aref estado-atual i j))
        (setf posicao-suposta (gethash valor-atual *mapa-posicoes*))
        (setf outro-valor (aref estado-atual (linha-posicao posicao-suposta) (coluna-posicao posicao-suposta)))
        (setf posicao-outro-valor (gethash outro-valor *mapa-posicoes*))
        (setf valor (+ valor (+ (abs (- (linha-posicao posicao-suposta) i))
                                (abs (- (coluna-posicao posicao-suposta) j)))))
        (when (and (= (linha-posicao posicao-suposta) (linha-posicao posicao-outro-valor))
                   (> (coluna-posicao posicao-suposta) (coluna-posicao posicao-outro-valor)))
          (setf valor (+ valor 2)))))))
